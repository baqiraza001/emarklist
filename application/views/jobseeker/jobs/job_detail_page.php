  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          <label for="message-text" class="col-form-label">Choose staff member :</label>
          <?php 
          $id=$job_detail['company_id'];
          $staff=$this->db->query("SELECT * FROM xx_staff WHERE bussiness_id='".$id."' AND service_type='".$job_detail['job_type']."' AND service_sub_type='".$job_detail['category']."' ")->result_array();
          ?>
          <select name="staff" class="form-control staff">
                <option value="">Staff</option>
                <?php foreach($staff as $country):?>
        ...
        <div class="form-group">
			  <label for="appt-time">Choose an appointment time (opening hours <?php echo $job_detail['start_time'].":00"; ?> to <?php echo $job_detail['end_time'].":00"; ?>): </label>
			  <!-- <input id="appt-time" type="time" name="appt-time" min="12:00" max="18:00">
			   -->
			   <select class="form-control appointment_time" name="appointment_time">
				<?php
				for ($i=$job_detail['start_time']; $i <$job_detail['end_time'] ; $i++) { 
				$count=$i;
				$countt=$i+1
				?>

				<option><?php echo $count.":00" ." - ". $countt.":00"; ?></option>				
				<?php 
				}
				?>
			  </select>
       ...
       <?php form_close(); ?>
					...
					<!--for service booking end-->
						...
						<p class="address">
							<strong>Price:</strong> <?= $job_detail['min_salary']; ?><?= $this->general_settings['currency']; ?> - <?= $job_detail['max_salary']; ?><?= $this->general_settings['currency']; ?>
						</p>
						...
						<p class="address">
							<strong>Expire Date:</strong> <?= date('d-m-Y', strtotime($job_detail['expiry_date'])); ?>
						</p>
