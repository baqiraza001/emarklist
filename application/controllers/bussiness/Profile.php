...
class Profile extends Main_Controller{
	...
	public function __construct(){
		...
		$this->load->model('bussiness/profile_model','profile_model');
	}
	...
	public function index(){
		...
		if ($this->input->post('update')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$result = $this->profile_model->update_bussiness($data,$emp_id);

				if ($result) {
					$this->session->set_flashdata('update_success',trans('personal_info_update_msg'));
					redirect(base_url('bussiness/profile'));
				}
			 }
		}
		else{
			...
			$emp_id = $this->session->userdata('employer_id');
			$data['emp_info'] = $this->profile_model->get_bussiness_by_id($emp_id);
			...
			$data['layout'] = 'bussiness/profile/profile_page';
	...
	public function change_password(){
		...
		if ($this->input->post('update_password')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url('bussiness/profile/change_password'),'refresh');
				...
				if($result) {
					...
					redirect(base_url('bussiness/profile/change_password'));
				}else{
		...
		}else
		{
			...
			$data['layout'] = 'bussiness/profile/change password';
			$this->load->view('layout', $data);
		}
	}
