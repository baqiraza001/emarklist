...
class Account extends Main_Controller {
...

	{
		...
		$this->load->model('bussiness/account_model', 'account_model');
...

	{
		...
		$data['total_posted_services'] = $this->account_model->total_posted_services();
		...
		$data['layout'] = 'bussiness/account/dashboard';
	...
	{	
		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url('bussiness/account/change_password'),'refresh');
				...
				if($result) {
					...
					redirect(base_url('bussiness/account/change_password'));
			...
			$data['emp_sidebar'] = 'bussiness/emp_sidebar'; // load sidebar for user

			$data['layout'] = 'bussiness/account/change_password_page';
