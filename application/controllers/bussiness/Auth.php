...
class Auth extends Main_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('bussiness/auth_model','auth_model');
	...
	public function login(){
		...
		if ($this->input->post('login')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($result) {
					...
					if($result['is_verify'] == 0){
						...
						redirect(base_url('bussiness/auth/login'));
						exit();
					}
					else if($result['employer_type']=='1'){
						$this->session->set_flashdata('error_login', 'Please Enter your email address!');
						redirect(base_url('bussiness/auth/login'));
						exit();
					}
					...
					if(!empty($this->session->userdata('last_request_page'))){
						...
						redirect(base_url('bussiness/profile'),'refresh');
					}
				}else{
			...
			$data['title'] = trans('bussiness_login');
	...
	public function registration()
	{
		...
		$data['countries'] = $this->common_model->get_countries_list(160); //get nigeria country 
		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url('bussiness/auth/registration'));
			}else{
				...
				$emp_id = $this->auth_model->insert_bussiness($emp_info); // Insert Employer Info & get ID
			...
			$data['layout'] = 'bussiness/auth/registration_page';
			$this->load->view('layout', $data);
		}
	}
	...
	public function forgot_password()
	{
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) 
			{
				...
				$this->session->set_flashdata('error', $data['errors']);
				redirect(base_url('bussiness/auth/forgot_password'));
			}
			$email = $this->input->post('email');
			...
			if($response){
				...
				$reset_link = base_url('bussiness/auth/reset_password/'.$pwd_reset_code);
			...
			$data['layout'] = 'bussiness/auth/forget_password_page';
			$this->load->view('layout', $data);
		}
	}
	...
	public function reset_password($id=0)
	{
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data['layout'] = 'bussiness/auth/reset_password_page';
				$this->load->view('layout', $data);
			}   
			else{
