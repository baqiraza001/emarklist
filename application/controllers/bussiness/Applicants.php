...
class Applicants extends Main_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('bussiness/applicant_model', 'applicant_model');
	...
	public function view($job_id){
		...
		$url= base_url("bussiness/applicants/view/".$job_id."/");
		...
		$data['layout'] = 'bussiness/applicants/job_applicants_page';
	...
	public function make_shortlist($id){
		...
		if($result){
			redirect(base_url('bussiness/applicants/shortlisted/'.$job_id), 'refresh');
		}
	}
	...
	public function get_shortlisted_user_profile()
	{
		if($this->input->post('user_id'))
		{
			...
			echo $this->load->view('bussiness/applicants/shortlisted_single_applicant_profile',$data,true);
		}
		else
		{
			redirect('bussiness/profile');
		}
	}
	...
	public function shortlisted($job_id){
		...
		$offset = ($this->uri->segment(5)) ? $this->uri->segment(5) : 0;
		$url= base_url("bussiness/applicants/shortlisted/".$job_id."/");
		...
		$data['layout'] = 'bussiness/applicants/shortlisted_applicants_page_apply_with_new_cv';
		$this->load->view('layout',$data);
	}
