   ...
   // $config['max_size']             = 2024;
	...
	public function addtocart(){
		if(!empty($this->session->userdata('user_id'))){
	        ...
	        if(!empty($check)){
	        	$add_category_by_user=$this->db->query("UPDATE xx_product_cart set `quantity` = `quantity`+$quantity, `price`= $totalprice,`created_by`= $user_id  where `product_id` = $product_id AND `user_id`= $user_id");
	        } else{
