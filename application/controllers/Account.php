...
class Account extends Main_Controller {
	...
	public function get_service_type_category()
	{
		$states = $this->db->select('*')->where('type_id',$this->input->post('service_type'))->get('xx_service_category')->result_array();
	    $options = array('' => 'Select Service Category') + array_column($states,'name','id');
	    $html = form_dropdown('category',$options,'','class="form-control select2" required');
		$error =  array('msg' => $html);
		echo json_encode($error);
	}
